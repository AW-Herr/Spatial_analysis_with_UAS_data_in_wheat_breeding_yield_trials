...
for (x in 1:length(aug.list)) {
  ...
  aug.base.mod <- mmer(BUAC ~ Check,
                       random= ~ R + C + IBLK + Name1,
                       rcov= ~ units,
                       data=aug.sub, verbose = F, nIters=30, tolParInv = 10, naMethodY = "include" , naMethodX = "include")
  
  save(aug.base.mod, file =  paste0("models\\mod-rowcol-aug-",aug.list[x],"-control.Rdata"))
  ...
  for (y in 1:length(index.list)) {
    ...
    save(aug.mod, file = paste0("models\\mod-rowcol-aug-",aug.list[x],"-",index.list[y],".Rdata"))
    ...
    write.csv(out, file = paste0("G:\\Chapter 4\\R\\Adjustments\\rowcol ",aug.list[x],".csv")) ############
...
#Alpha Trials
#Control Base Model
#alpha.base <- lm(BUAC ~ BLOC + IBLK + Name1, data = alpha)
...
for (x in 1:length(alpha.list)) {
  ...
  alpha.base.mod <- mmer(BUAC ~ Name1,
                       random= ~ R + C + BLOC + IBLK,
                       rcov= ~ units,
                       data=alpha.sub, verbose = F, nIters=30, tolParInv = 10, naMethodY = "include", naMethodX = "include")
  
  save(alpha.base.mod, file = paste0("models\\mod-rowcol-alpha-",alpha.list[x],"-control.Rdata"))
  ...
  for (y in 1:length(index.list)) {
    ...
    save(alpha.mod, file = paste0("models\\mod-rowcol-alpha-",alpha.list[x],"-",index.list[[y]],".Rdata"))
    ...
    write.csv(out, file = paste0("G:\\Chapter 4\\R\\Adjustments\\rowcol ",alpha.list[x],".csv")) ############
...
write.csv(output, file = "Row-Col Fixed Effect Model.csv", row.names = F)
```
